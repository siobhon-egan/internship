...
usearch10="usearch10"
# Enter name of usearch9 to match path
# Enter name of usearch10 to match path
# Enter raw data directorry
raw_data="raw_data"
# Enter directory for fastq output information
...
derep_dir="6.dereplicated_sequences"
...
SF_derep="9.dereplicated_filtered_data"
#Enter directory for unoise files
unoise_all="10.unoise_all"

##########################################################################################
# DO NOT EDIT BELOW THIS LINE
##########################################################################################
echo %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
echo Raw fastq sequence information
echo %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for file11 in ${labeled_data}/*.fasta
	...
	$usearch9.2 -search_exact ${file11} -db "${low_abund_seqs}/$(basename "$file11" .fasta)_DR.fasta" -strand plus -notmatched "${SF}/$(basename "$file11" .fasta).fasta"
done
...
$usearch10 -unoise3 all_samples_SF_DR.fasta -zotus unoise_zotus.fasta -tabbedout unoise1_tab.txt

$usearch10 -fastx_relabel unoise_zotus.fasta -prefix Otu -fastaout unoise_zotus_relabelled.fasta -keep_annots

$usearch10 -otutab all_samples_SF_DR.fasta -zotus unoise_zotus_relabelled.fasta -otutabout unoise_otu_tab.txt -biomout unoise_otu_biom.json -mapout unoise_map.txt -notmatched unoise_notmatched.fasta -dbmatched dbmatches.fasta -sizeout

gzip all_samples_SF_DR.fasta
gzip unoise_zotus.fasta
