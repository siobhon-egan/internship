...
# Removing working directory
	...
	# Zip raw fastq files in {raw_data}
	cd ${raw_data}
	gzip *.fastq
	cd ..
...
for file3 in ${merged_data}/*.fastq
	...
	usearch9.2 -fastq_filter ${file3} -fastaout "${QF}/$(basename "$file3" .fastq)_QF.fasta" -fastq_maxee_rate ${error_rate} -fastq_minlen ${minlen}
done
...
##########################################################################################
	...
	echo Copy terminal output to a .txt for a log file
