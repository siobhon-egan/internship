#!/bin/bash
# Script for Nimbus linux machine
...
for file1 in ${raw_data}/*R1_001.fastq
	...
	usearch9.2_linux -fastq_mergepairs ${file1} -reverse "${raw_data}/$(basename -s R1_001.fastq ${file1})R2_001.fastq" -fastqout "working1/$(basename "$file1")" -fastq_minovlen ${overlap}
done
...
for file3 in ${merged_data}/*.fastq
	...
	usearch9.2_linux -fastq_filter ${file3} -fastaout "${QF}/$(basename "$file3" .fastq).fasta" -fastq_maxee_rate ${error_rate} -fastq_minlen ${minlen}
done
#*****************************************************************************************
...
for file4 in ${QF}/*.fasta
	...
	usearch9.2_linux -search_oligodb ${file4} -db fwd_primer_db.fasta -strand both -matched "${seqs_w_fwd_primer}/$(basename ${file4})" -notmatched "${seqs_wo_fwd_primer}/$(basename ${file4})"
done
#*****************************************************************************************
...
for file5 in ${seqs_w_fwd_primer}/*.fasta
	...
	usearch9.2_linux -search_oligodb ${file5} -db rev_primer_db.fasta -strand both -matched "${seqs_w_fwd_and_rev_primer}/$(basename ${file5})" -notmatched "${seqs_w_fwd_butnot_rev_primer}/$(basename ${file5})"
done
#*****************************************************************************************
...
for file6 in ${seqs_w_fwd_and_rev_primer}/*.fasta
	...
	usearch8.0_linux32 -search_pcr ${file6} -db both_primers_db.fasta -strand both -maxdiffs ${pcr_missmatches} -pcr_strip_primers -ampout "${trimmed_data}/$(basename ${file6} .fasta).fasta"
done
#*****************************************************************************************
...
for file9 in ${labeled_data}/*.fasta
	...
	usearch9.2_linux -fastx_uniques ${file9} -fastaout "${derep_dir}/$(basename "$file9" .fasta).fasta" -sizeout
done
#*****************************************************************************************
...
for file10 in ${derep_dir}/*.fasta
	...
	usearch9.2_linux -sortbysize ${file10} -fastaout "${low_abund_seqs}/$(basename "$file10" .fasta).fasta" -maxsize ${maxsize}
done
#*****************************************************************************************
...
for file11 in ${labeled_data}/*.fasta
	...
	usearch9.2_linux -search_exact ${file11} -db "${low_abund_seqs}/$(basename "$file11" .fasta).fasta" -strand plus -notmatched "${SF}/$(basename "$file11" .fasta).fasta"
done
#*****************************************************************************************
...
for file15 in ${SF}/*.fasta
	...
	usearch9.2_linux -fastx_uniques ${file15} -fastaout "${SF_derep}/$(basename "$file15" .fasta).fasta" -sizeout
done
#*****************************************************************************************
