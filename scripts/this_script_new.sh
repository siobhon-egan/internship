...
for file1 in ${raw_data}/*R1_001.fastq
	...
	usearch9.2 -fastq_mergepairs ${file1} -reverse "${raw_data}/$(basename -s R1_001.fastq ${file1})R2_001.fastq" -fastqout "working1/$(basename "$file1")" -fastq_maxdiffs ${maxdiffs} -fastq_minovlen ${overlap} -report ${merged_data}/2d_merging_seqs_report.txt -tabbedout ${merged_data}/2e_tabbedout.txt
done
